...
9. 기존 발송 이력 제거
10. 상세 페이지 본문 추출
11. Node.js/NestJS 필수 키워드 기반 1순위 후보 분리
12. Node.js/NestJS 후보가 30개 미만이면 백엔드/서버 개발 포지션으로 보충
13. 회사명 기준 중복 제거
14. 규칙 기반 적합도 점수 계산
15. 1순위 후보 점수 높은 순 정렬 후 보충 후보를 점수순으로 추가
16. 최대 30개만 Slack 발송
17. 채용공고 Slack 발송 성공 시 실제 발송한 URL만 `seen-items.json`에 저장
...
Node.js/NestJS 명시 공고가 부족한 경우에는 백엔드/서버 개발 포지션을 보충 공고로 포함한다. 보충 공고는 Slack 메시지의 주의 포인트에 보충 포지션임을 표시한다.
